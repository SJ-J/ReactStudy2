...
- `createContext`/`useContext`(`Context1`)로 `players`, `remanet` 데이터를 하위 컴포넌트에 전달
...
| `TabContent` | `pages/Detail.jsx` | 선택된 탭(`tabs`)에 따라 내용0/1/2 렌더, `Context1`에서 state 조회해 탭0에 표시, 탭 변경 시 페이드인 트랜지션 적용 |
